...
<body>
  ...
  <?php if ($_GET['type'] === 'user'): ?>
    ...
    <form action="" method="post">
      ...
      <p>自己紹介</br><?php echo nl2br(h($app->getValues()->user->description)); ?></p>
  ...
  <?php if ($_GET['type'] === 'article'): ?>
    ...
    <form action="" method="post">
      ...
      <p>記事紹介</br><?php echo nl2br(h($app->getValues()->article->description)); ?></p>
  ...
  <?php if ($_GET['type'] === 'comment'): ?>
    ...
    <form action="" method="post">
      <p>コメント</br><?php echo nl2br(h($app->getValues()->comment->comment)); ?></p>
      <input type="hidden" name="id" value="<?php echo h($_GET['id']); ?>">
      <input type="hidden" name="token" value="<?php echo h($_SESSION['token']); ?>">
      <input class="btn" type="submit" value="削除">
    </form>
